...
from stock_screener import calc_rsi, calc_macd, _get_jquants_id_token
...
# ── グリッド ──────────────────────────────────────────────────────────────────
GRID = {
    "rsi_period":  [14, 21, 30],                  # RSI期間 (3)
    "rsi_hi":      [25.0, 30.0, 35.0, 40.0],     # RSI上限 (4)
    "macd_filter": [True, False],                  # MACD < シグナル必須 (2)
    "max_hold":    [10, 15, 20],                   # 最大保有日数 (3)
...
def preprocess(ticker: str, df_raw: pd.DataFrame,
    ...
    # MACD（12/26/9）
    close_s  = pd.Series(c)
    macd_s, sig_s = calc_macd(close_s)
    macd_arr = macd_s.values
    sig_arr  = sig_s.values
    macd_below_sig = (macd_arr < sig_arr) & ~np.isnan(macd_arr) & ~np.isnan(sig_arr)
    ...
    # RSI を複数期間で計算
    rsi_arrays: dict[int, np.ndarray] = {}
    for period in [14, 21, 30]:
        rsi_arr = calc_rsi(close_s, period=period).values
        rsi_arrays[period] = rsi_arr
        for hi in [25.0, 30.0, 35.0, 40.0]:
            flags[f"r{period}_{int(hi)}"] = (rsi_arr <= hi) & ~np.isnan(rsi_arr)
...
def _calc_rets(highs, lows, opens, closes, vidx, e_arr, s_arr, t_arr, max_hold):
    n        = len(closes)
...
def _get_mask(flags, rsi_period, rsi_hi, macd_filter, mktcap_max):
    m = flags["base"] & flags[f"r{rsi_period}_{int(rsi_hi)}"]
    if macd_filter:
        m = m & flags["macd_below"]
...
def run_grid(all_proc: list[dict], trading_days: int) -> list[dict]:
    ...
    for ci, combo_vals in enumerate(combos, 1):
        ...
        for td in all_proc:
            m = _get_mask(
                td["flags"],
                p["rsi_period"], p["rsi_hi"],
                p["macd_filter"], p["mktcap_max"],
            )
            vidx = np.where(m)[0]
            if len(vidx) == 0:
                continue
...
def _fmt_params(r: dict) -> str:
    ...
    macd = "MACD↓" if r["macd_filter"] else "MACD--"
    return (f"RSI({r['rsi_period']})≤{r['rsi_hi']:.0f}  {macd}  "
            f"保有{r['max_hold']}日  {mc}  RR{r['rr']}")
...
def print_results(results: list[dict]) -> None:
    ...
    print(f"\n{'='*90}")
    print(f"【ニッポン・オプティマライザー（NOA）グリッドサーチ 結果】")
    print(f"  総計: {len(results)} 通り  |  合格: {len(qualified)} 通り")
    print(f"  評価基準: WR≥{CRITERIA_WR}%  PF≥{CRITERIA_PF}  件/日≥{CRITERIA_SPD}")
    print(f"  スクリーナー条件: RSI(30)≤30 / MACD下向き / 保有10日 / 時価総額≤300億 / RR2.0")

    hdr = f"  {'条件':<52}  {'勝率':>6}  {'PF':>5}  {'EV':>7}  {'件/日':>6}  {'Score':>6}"
    sep = "  " + "-" * 87
    ...
    if qualified:
        ...
        print(hdr); print(sep)
        for r in top:
            print(f"  {_fmt_params(r):<52}  {r['wr']:>5.1f}%  {r['pf']:>5.2f}"
        ...
        print(f"  MACDフィルター: {'あり（MACD < シグナル）' if best['macd_filter'] else 'なし'}")
        print(f"  最大保有   : {best['max_hold']}日")
...
def main():
    ...
    print("=" * 90)
    print("ニッポン・オプティマライザー（NOA）グリッドサーチ 開始")
